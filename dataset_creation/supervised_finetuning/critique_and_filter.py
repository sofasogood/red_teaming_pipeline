...
""",
    ...
    idx = (dataset.critique.isna()) | (
        dataset.critique.notna() & (dataset["critique"].str.len() < 10)
    )
    # Find first occurrence
    first_matching_index = idx[idx].index[0]
    for row_indexer in range(first_matching_index, first_matching_index + 1000):
        condition = (dataset.revised_prompt.notna()) & (
            dataset["revised_prompt"].str.len() > 10
        )
        if condition[row_indexer]:
            ...
            dataset["critique_prompt"].iloc[row_indexer] = (
                constitutional_principle.critique_prompt
            )
            ...
            print("====== ORIGINAL ======")
            print(dataset["question"][row_indexer])
