...
from dataset_creation.persuasion_techniques.persuation_techniques import (
    sample_technique,
)
from dataset_creation.data_processing_functions.data_creation import (
    extract_question,
    PersuasiveRevision,
)
import os
...
def process_single_row(
    row_data: Dict[str, Any], model_name: str = "microsoft/wizardlm-2-8x22b"
) -> Dict[str, Any]:
    ...
    try:
        ...
        if (
            "persuation_technique" in row
            and isinstance(row["persuation_technique"], str)
            and len(row["revised_prompt"]) >= 10
        ):
            return None
        ...
        result = PersuasiveRevision(
            persuation_technique, row["question"], model_name=model_name
        ).process_question()
...
def process_dataset_parallel(
    ...
    for result in tqdm(
        pool.imap_unordered(process_func, rows_to_process), total=batch_size
    ):
...
def save_checkpoints(dataset: pd.DataFrame, results: list):
    ...
    for result in results:
        if result is not None:
            ...
            data = result["data"]
            dataset.loc[row_index, "persuation_technique"] = data[
