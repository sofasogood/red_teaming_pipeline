...
if __name__ == "__main__":
    ...
    for row_index in range(first_matching_index, first_matching_index + 10):
        ...
        if (
            ...
            or not isinstance(row["responseA"], str)
            or len(row["responseA"]) < 10
        ):
            rows_to_process.append((row_index, row))

    # Process rows in parallel
    num_processes = min(
        cpu_count(), len(rows_to_process)
    )  # Don't create more processes than rows
    print(f"Starting parallel processing with {num_processes} processes")

    with Pool(processes=num_processes) as pool:
        results = pool.map(
            partial(process_single_row, save_interim=True), rows_to_process
        )
    ...
    for result in results:
        ...
        dataset_red_team_train_subset.loc[row_index, "responseA"] = row_data[
            "responseA"
        ]
        dataset_red_team_train_subset.loc[row_index, "responseB"] = row_data[
            "responseB"
        ]
